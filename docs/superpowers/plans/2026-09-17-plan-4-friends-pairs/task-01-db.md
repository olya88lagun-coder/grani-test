...
Локальная база `pnpm dev:db` применяет миграции только при старте: если она уже запущена, остановить её через Ctrl+C (не принудительным завершением процесса — PGlite может повредить `.dev-db/`) и запустить заново.

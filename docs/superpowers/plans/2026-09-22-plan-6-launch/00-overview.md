...
> **Статус: код выполнен 2026-09-23, выкладка впереди.** 521 тест + 13 сквозных (Playwright, с работающим воркером); покрытие: строки 94,6%, выражения 92,9%, функции 91,4%, ветви 89,3%. Сборки сайта и воркера без предупреждений; production-сборка проверена локально (страницы, карточка со шрифтами, dev-вход и поддельная оплата отвечают 404). Осталось: вычитка статей владелицей, настройка сервера, выкладка и ручная проверка на телефоне (задачи 7 и 8).
...
## Изменение по ходу плана: вход только через VK ID

Решение от 2026-09-24. Telegram — иностранный сервис, поэтому вход через него и уведомления бота означают трансграничную передачу персональных данных: отдельное уведомление в Роскомнадзор, перечень стран и риск ограничения. Все остальные получатели данных российские (VK ID и сообщество ВКонтакте, ЮKassa, YandexGPT или GigaChat, Яндекс.Метрика, сервер Timeweb в Москве), поэтому в первой версии Telegram выключается, и заявление в РКН обходится без трансграничного раздела.

Что это значит в коде: `TELEGRAM_BOT_TOKEN` и `TELEGRAM_BOT_USERNAME` стали необязательными; без них на странице входа остаётся только VK ID, маршрут виджета отвечает 404, а `loginWithTelegram` возвращает `telegram_disabled`. Код Telegram и отправка уведомлений ботом остались в репозитории и включаются обратно двумя переменными — вместе с уведомлением в РКН о трансграничной передаче и возвратом Telegram в политику и согласие.

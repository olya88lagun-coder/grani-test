...
Затем владелица открывает `nano /opt/grani/.env` и вписывает ID приложения VK ID вместо `PASTE_VK_CLIENT_ID`. Остальные переменные с пояснениями перечислены в `deploy/env.example`.
...
**Вход через Telegram в первой версии выключен.** Telegram — иностранный сервис, поэтому вход через него означает трансграничную передачу персональных данных и отдельное уведомление в Роскомнадзор. Без переменных `TELEGRAM_BOT_TOKEN` и `TELEGRAM_BOT_USERNAME` сайт показывает только вход через VK ID, а маршрут виджета отвечает 404. Код Telegram остался в репозитории: чтобы включить его позже, нужно задать обе переменные, подать уведомление о трансграничной передаче и вернуть Telegram в политику и согласие.

Если Telegram когда-нибудь включат: токен из старой переписки скомпрометирован, сначала `/revoke` в @BotFather, затем `/setdomain` → `grani-test.ru`. Одновременно токен может опрашивать только один процесс: локальный воркер запускать с `NOTIFICATIONS_DRY_RUN=1`.

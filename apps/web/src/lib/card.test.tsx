...
import { CARD_COLORS, CARD_SIZE, cardElement, loadCardCrystal, loadCardFonts, manualCardElement } from "./card";
...
test("renders a story-sized PNG", async () => {
  const element = cardElement({ name: "Искра", keywords: typeKeywords("+-++"), crystal: await loadCardCrystal() });
...
test("fits a long gendered name", async () => {
  const element = cardElement({ name: "Тихая хранительница", keywords: typeKeywords("-+-+"), crystal: await loadCardCrystal() });
  expectStoryPng(await pngBytes(new ImageResponse(element, { ...CARD_SIZE, fonts: await loadCardFonts() })), TYPE_CARD_MAX_BYTES);
}, 30_000);
